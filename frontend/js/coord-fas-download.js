...
window.downloadCoordFasByPrn = async function downloadCoordFasByPrn() {
  ...
  try {
    ...
    previewEl.innerHTML = `
      <div style="display:flex;align-items:center;gap:0.75rem;padding:0.85rem 1rem;background:rgba(5,150,105,0.08);border:1px solid rgba(5,150,105,0.2);border-radius:0.75rem;margin-bottom:1rem">
        <span style="font-size:1.25rem">👤</span>
        <div>
          <div style="font-weight:700;color:var(--uni-blue-dark)">${escapeHtml(data.name)}</div>
          <div style="font-size:0.88rem;color:var(--text-muted)">PRN: ${escapeHtml(data.prnNumber || prn)} · ${escapeHtml(data.email || '')}</div>
        </div>
      </div>`;
